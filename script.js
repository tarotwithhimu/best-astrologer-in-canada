...
document.querySelectorAll('a[href*="wa.me"]').forEach(btn => {
    btn.addEventListener('click', () => {
        console.log('WhatsApp click - Best Astrologer in Canada');
    });
});
...
if (locationSearch) {
    ...
    emptyMsg.className = 'location-search-empty';
    emptyMsg.textContent = 'No matching city found — try a different spelling, or WhatsApp us and we\u2019ll confirm coverage.';
    locationSearch.closest('.container').appendChild(emptyMsg);
    ...
    locationSearch.addEventListener('input', function () {
        ...
        emptyMsg.classList.toggle('visible', q.length > 0 && !anyVisibleTotal);
    });
}

document.addEventListener('DOMContentLoaded', () => {
    console.log('Best Astrologer in Canada — Himu');
});
